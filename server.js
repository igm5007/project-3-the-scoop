...
//called when case is '/comment: POST'
//creates a new comment, assigns an ID, adds comment to article comment IDs and user comment IDs
function createComment(url, request){
  //gets input data, sets requestComment to request.body.comment if request.body is truthy
  const requestComment = request.body && request.body.comment;
  
  //defines the response object
  const response = {};

  //fails if requestComment body/username/article aren't set
  //also fails if the article or commenting user don't exist
  if (requestComment && requestComment.body && requestComment.username &&
    ...
    //creates the comment object and its key/pairs
    const comment = {
    ...
    //pushes this comment objects id to its comment id for the article it is associated with
    database.articles[comment.articleId].commentIds.push(comment.id);
    
    //pushes this comment objects id to its comment id for the user it is associated with
    database.users[comment.username].commentIds.push(comment.id);

    //fills the response object set early
...
//called as the PUT of '/comments/:id' to update existing comments
function updateComment(url, request){
  //seperates the comments id from the comments url
  const id = Number(url.split('/').filter(segment => segment)[1]);

  //finds the comment in the database 
  const savedComment = database.comments[id];

  //opens the existing body in the text editor
  const requestComment = request.body && request.body.comment;
  const response = {};

  
  //if statement that fails when the id or requestComment varaibles are both undefined
  // and fails if the id and requestComment exist but the savedComment does not
  if (!id || !requestComment) {
    response.status = 400;
  } else if (!savedComment) {
    response.status = 404;
  } else {
    //resets the savedComment.body to the new text in the text editor
    savedComment.body = requestComment.body || savedComment.body;

    //saves the newly set savedComment to the response body
